...
class VideoPage extends Component {
    ...
    function onPlayerStateChange(event) {
      ...
      self.setState({ videoState: event.data }, () => {
        switch (event.data) {
          case 1:
            self.videoProgressWatcher();
            if (Math.abs(self.state.videoPlayer.getCurrentTime() < self.timeAtPause)) {
              console.log('CURRENT TIME', self.state.videoPlayer.getCurrentTime());
              console.log('TIME AT PAUSE', self.timeAtPause);
              console.log('##TEST', Math.abs(self.state.videoPlayer.getCurrentTime() - self.timeAtPause));
            ...
            } else if (self.currentClip && self.currentClip.playbackType === 'extended') {
              self.currentClip.stop();
            }
            break;
          case 2:
            ...
            self.timeAtPause = self.state.videoPlayer.getCurrentTime();
            if (self.currentClip && self.currentClip.playbackType === 'inline') self.currentClip.pause();
            break;
          default:
            if (self.watcher) {
              clearInterval(self.watcher);
  ...
  videoProgressWatcher() {
    ...
    // console.log('AUDIOCLIPS', this.state.audioClips);
    // this.audioClipsCopy = this.state.audioClips.slice();
    ...
    this.watcher = setInterval(() => {
            ...
            switch (this.audioClipsCopy[i].playback_type) {
              ...
              case 'extended':
                ...
                this.currentClip = new Howl({
                  src: [this.audioClipsCopy[i].url],
                  html5: true,
                  onend: () => this.state.videoPlayer.playVideo(),
                });
      ...
      //       break;
      //     case 1:
      //       if (Math.abs(currentVideoProgress - timeAtPause) > .1) {
      //         this.currentClip.stop();
      //         this.currentClip = null;
      //         this.audioClipsCopy = this.state.audioClips.slice();
      //         console.log('different time!');
      //         if (this.currentClip && this.currentClip.playbackType === 'extended') {
      //           this.state.videoPlayer.pauseVideo();
      //         }
      //       }
      //       if (this.currentClip && this.currentClip.playbackType === 'inline') {
      //         this.currentClip.play();
      //       } else if (this.currentClip && this.currentClip.playbackType === 'extended') {
      //         this.currentClip.stop();
      //       }
      //       break;
      //     case 2:
      //       timeAtPause = currentVideoProgress;
      //       if (this.currentClip && this.currentClip.playbackType === 'inline') this.currentClip.pause();
      //       break;
      //     default:
      //     console.log('default');
      //   }
